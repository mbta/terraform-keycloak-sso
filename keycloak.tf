...
resource "aws_ecs_task_definition" "keycloak-ecs-taskdef" {
  ...
  memory                   = local.keycloak_task_memory
  cpu                      = local.keycloak_task_cpu
  execution_role_arn       = aws_iam_role.keycloak-ecs-execution-role.arn
  task_role_arn            = aws_iam_role.keycloak-ecs-task-role.arn
...
resource "aws_ecs_service" "keycloak-service" {
  ...
  depends_on = [
    ...
    aws_iam_role.keycloak-ecs-execution-role,
    aws_iam_role.keycloak-ecs-task-role,
